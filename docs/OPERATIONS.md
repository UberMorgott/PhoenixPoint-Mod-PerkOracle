...
- The mod writes **no** runtime files into its own folder: swap history lives
  inside the savegame via the game's mod-instance-data API
  (`ModGeoscape.RecordGeoscapeInstanceData`). So `/MIR` has nothing of the
  player's to destroy. If a future change ever writes a runtime file to
  `Entry.Directory`, add it to a `/XF` exclusion here before deploying again.
