...
Date: 2026-06-08 · Status: **road map / ideas** (not approved, not scheduled) · Mod: PerkOracle (standalone, ns `Morgott.PerkOracle`, Dependencies `[]`)
...
- Extend the existing read-only perk wiki beyond the ability-progression screen onto three more surfaces, turning PerkOracle into a broader in-game "soldier wiki":
  ...
  - **Recruit preview (B)** — on hiring/recruitment surfaces (haven recruits, base personnel), click a candidate soldier to open the **full character page** (3D model, all panels, stats, progression) like the squad-management view, **read-only** (no inventory changes). The existing perk highlight + candidate wiki ride along on its progression screen.
...
- The page is **read-only**: the player can look at everything but **cannot change inventory** (no equip/unequip/drag-drop) or otherwise mutate the candidate.
...
- **Primary: route the recruit into the full character/management view in a preview mode.**
  - Open the same character screen the roster uses, populated from the candidate soldier (not yet in the roster).
  - Enter a **preview/read-only flag** for the duration: suppress all inventory mutation (equip/unequip, drag-drop, context actions) while leaving viewing, the 3D model, stat panels, and the progression/perk wiki fully functional.
  - Read-only enforcement options (pick at implementation): Harmony-prefix the equip/drag handlers to no-op when the preview flag is set, or disable/hide the equip-interaction components on the cloned screen. Prefer the least invasive that fully blocks mutation.
- **Fallback if the full screen proves too heavy/unsafe:** the lightweight perk wiki popup from the original proposal (class perks + personal pool in one `PerkWikiPanel`). Keep as a documented Plan B, not the goal.
...
- **Read-only guard (new).** A preview flag + the guards (prefix/disable) that block every inventory/equipment mutation path while the preview is open. Must be airtight — a read-only preview that can accidentally equip is a bug, not a feature.
...
- **Every inventory-mutation entry point** on that screen, so the read-only guard covers them all (equip, unequip, drag-drop, quick-actions, context menus).
- **3D model / actor instantiation** for a non-hired candidate, and its clean teardown on close.
- **Whether a recruit's personal perks are pre-rolled at list time or only at hire** — if pre-rolled, the progression screen shows exact perks; if not, unrolled slots fall back to the pool preview.
...
### Data source (curated — the game has no "signature" flag)

- A and B are **pure read-out**: the game forms the perks/pools, the mod only reads and displays them. C is different — the game does **not** tag any ability as "this is the merc's gimmick", so there is nothing native to read.
- So C owns a **curated mapping**: `characterDefId -> gimmick blurb` (a short authored line, or an `abilityId` whose game-localized description is pulled in). Maintained per known unique merc, lives in the mod.
- Optional heuristic auto-detect (ability not in class track nor personal pool) can flag unlisted mercs, but it cannot author the explanatory blurb, so the curated text stays primary.

### New pieces

- **Gimmick registry (curated, mod-owned).** Small data table `characterDefId -> blurb` (+ optional `abilityId` to source localized description). Primary source of truth; easy to extend.
...
- **Localization.** The appended blurb should be localized: either authored per-language rows in the existing 8-language CSV (e.g. `PERKORACLE_GIMMICK_<mercKey>`), or sourced from the game's own localized ability description when an `abilityId` is used. Optional small prefix label term (e.g. `PERKORACLE_GIMMICK_PREFIX` → "Special: ").
...
- Which gimmick belongs to each known merc — manual catalogue work for the registry (blurb text and/or the `abilityId`).
...
- **Read-only preview flag (Feature B).** A single process-wide "preview is open" flag the inventory-mutation guards read. Centralize it so every guard (and the existing swap path, which must also stay disabled in preview) checks one source of truth.
...
- **Recruit page scope:** full character/management view in read-only mode (preferred), or the lightweight perk-wiki popup fallback if the full screen is too risky/heavy?
- **Recruit personal perks:** confirm pre-rolled vs rolled-at-hire — drives exact-vs-pool preview on the progression screen (the single biggest unknown for Feature B).
- **Read-only strictness:** which exact actions to block — just inventory/equip, or also any other mutation (rename, dismiss, ability assignment)? Default: block all mutation.
- **Merc gimmick registry (Feature C):** the game has no "gimmick" flag, so the mod owns a curated `characterDefId -> blurb` table appended to the bio. Confirm: authored per-merc blurbs in the CSV (primary) vs pulling a game ability description by `abilityId`, and is the manual catalogue worth maintaining?
...
- **Phase 1 — investigation spike.** Resolve every **[verify]** anchor (class-track accessor, full subclass enumeration + which the screen omits, recruit + merc-shop UI controllers, recruit roll timing, character-view-for-non-roster-soldier, inventory-mutation entry points, biography text seam, merc keying). No feature code; output a follow-up locked spec per feature.
...
- **Phase 4 — Feature B (recruit full-page preview).** Highest risk (opening a full management view for a non-roster soldier + airtight read-only guard + 3D actor lifecycle). Do last, after the others prove the surrounding pieces. The perk highlight + wiki ride along inside it for free.

## Out of scope (this proposal)

- Any change to perks, classes, abilities, or balance — strictly read-only preview/highlight.
- Perk-swap or any mutation on the new surfaces (the existing swap stays confined to the post-hire progression screen and is disabled while a preview is open).
